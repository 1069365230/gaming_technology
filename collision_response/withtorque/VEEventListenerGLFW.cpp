...
namespace ve {
	...
	*/
	bool VEEventListenerGLFW::onKeyboard(veEvent event) {
		...
		VESceneNode *pScene, *throwingCube;
		VESceneNode *throwingCubeParent;
		int cubeId;
		Mycube *cube;

		switch (event.idata1) {
		case GLFW_KEY_A:
			//event.dt time passed(diff in time)
			//pass forces to onFrame
		...
		case GLFW_KEY_SPACE:
			//last frame presses?
			if (pressed == false) {
				//detect has been pressed during last frame
				VECHECKPOINTER(pScene = getSceneManagerPointer()->getSceneNode("Level 1"));
				//creating a new parent with a new number 
				throwingCubeParent = getSceneManagerPointer()->createSceneNode("Throwing Cube Parent" + std::to_string(cubenumber), pScene, glm::mat4(1.0));
				
				std::cout << "vector size before: " << allmycubes.size()<<endl;
				VECHECKPOINTER(throwingCube = getSceneManagerPointer()->loadModel("The Throwing Cube" + std::to_string(cubenumber), "media/models/test/crate2", "cube.obj"));
				//cubeId = cubenumber;
				pressednumber = cubenumber;
				cubenumber++;
				
				//throwingCube->setPosition(glm::vec3(0.0f, 10.0f, 3.0f));
				throwingCubeParent->setPosition(glm::vec3(0.0f, 5.0f, 3.0f));

				throwingCubeParent->addChild(throwingCube);
				float mass = 1;
				cube = new Mycube(throwingCube, throwingCubeParent, mass);
				//inertia doesnt change overtime, so I setup here
				cube->setInertia(mass, glm::vec3(0.0f, 1.0f, 3.0f));
				cube->setTime(event.dt);

				cube->setAngularMomentum(glm::vec3(6.0, 0.0, 0.0));
				allmycubes.push_back(cube);
				std::cout << "after: "  << allmycubes.size() << endl;
			}
			pressedFrame = true;
	...
	*/
	void VEEventListenerGLFW::onFrameEnded(veEvent event) {
		if (pressedFrame == true) {
			pressed = true;
		}
		else {
			pressed = false;
		}
		//pressed = pressedFrame;
		pressedFrame = false;
		if (m_makeScreenshot) {
