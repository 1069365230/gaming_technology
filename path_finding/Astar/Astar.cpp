...
class Node {
	private:
		...
		float estimatedCost = 9999;
		float totalEstimatedCost = 9999;
	public:
		int getX();
		int getY();
		float getCost();
		...
		//A*
		float getEstimated();
		void setEstimated(float n);
		float getTotalEstimated();
		void setTotalEstimated(float n);
};
...
float Node::getEstimated() {
	return estimatedCost;
}

void Node::setEstimated(float n) {
	this->estimatedCost = n;
}

float Node::getTotalEstimated() {
	return totalEstimatedCost;
}

void Node::setTotalEstimated(float n) {
	this->totalEstimatedCost = n;
}
...
Node* findSmallestCost(list<Node*> l) {
	...
	string smallestNode;
	for (i = l.begin(); i != l.end(); i++) {
		float cost = (*i)->getTotalEstimated();
		if (cost < min) {
			min = cost;
			smallestNode = (*i)->getName();
		}
	}
	cout << "min is: " << min << endl;
	cout << "choosed: " << smallestNode << endl;
	Node* node = findNode(smallestNode, l);
	return node;
	/*for (i = l.begin(); i != l.end(); i++) {
...
void setupEsimatedCost(list<Node*>& l, string goal) {
	Node* node = findNode(goal, l);
	int destinationX = node->getX();
	int destinationY = node->getY();
	cout << "X: " << destinationX << "Y: " << destinationY << endl;
	list<Node*>::iterator i;
	for (i = l.begin(); i != l.end(); i++) {
		int x = (*i)->getX();
		int y = (*i)->getY();
		float estimate = sqrt(pow(destinationX - x, 2) + pow(destinationY - y, 2));
		(*i)->setEstimated(estimate);
		cout << "NODE NAME: " << (*i)->getName() << endl;
		cout <<"ESTIMATE: "<< (*i)->getEstimated() << endl;
		cout << "-----------------------" << endl;
	}
}
...
void processNode(Node* currentNode, list<string> neighbourNames, list<Node*>  & openList, list<Node*> & unvisited, list<Node*>& closed) {
	for (string name : neighbourNames) {
		...
		if (checkInUnvisit(name, unvisited) == false) {
			...
			float storedCost = node->getTotalEstimated();
			float newCost = currentNode->getCost() + node->getWeight() + node->getEstimated();
			if (newCost < storedCost) {
				cout << "new cost is smaller than stored:++++++++++" << endl;
				//node->setCost(newCost);
				node->setTotalEstimated(newCost);
		...
		//A*
		node->setTotalEstimated(node->getCost() + node->getEstimated());
...
void dijkstra(list<Node*> & open, list<Node*> & closed, list<Node*> & unvisited) {
	cout << "------------------------------------------------------" << endl;
	//find smallest now has to add the estimated cost
...
int main() {
	...
	setupEsimatedCost(unvisited, "B5");

	list<Node*> openList;
	list<Node*> closedList;
	//initial stage
	Node* start = findNode("B2", unvisited);
	//set the sofarcost to 0(might need to change)
	start->setCost(0);
	start->setTotalEstimated(start->getCost() + start->getEstimated());
	...
	*/
	...
	while (closedHasGoal("B5", closedList) == false) {
		dijkstra(openList, closedList, unvisited);
	...
	string path = getPath("B2", "B5", closedList);
	cout <<"Path: " << path << endl;
	cout << "unvisited now: " << endl;
	printList(unvisited);
	return 0;
